...
export default function ContentEditor({ id }: { id?: string }) {
  ...
  return <form onSubmit={save} className="mt-8 grid gap-6 lg:grid-cols-[1fr_280px]">
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-3"><input required value={content.slug} onChange={(event) => update("slug", event.target.value)} placeholder="url-slug" className="rounded-xl border border-white/10 bg-[#0e1318] px-3 py-3 text-sm outline-none" /><input required value={content.category} onChange={(event) => update("category", event.target.value)} placeholder="Category" className="rounded-xl border border-white/10 bg-[#0e1318] px-3 py-3 text-sm outline-none" /><select value={content.contentType} onChange={(event) => setContent((current) => ({ ...current, contentType: event.target.value as "guide" | "news" }))} className="rounded-xl border border-white/10 bg-[#0e1318] px-3 py-3 text-sm"><option value="news">News</option><option value="guide">Guide</option></select></div>
